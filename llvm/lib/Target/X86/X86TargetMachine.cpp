...
static std::string computeDataLayout(const Triple &TT) {
  ...
  if (!TT.isArch64Bit() || TT.isX32() || TT.isOSSXFI())
    Ret += "-p:32:32";
  ...
  if (TT.isArch64Bit() || TT.isOSWindows() || TT.isOSSXFI())
    Ret += "-i64:64";
...
void X86PassConfig::addPreEmitPass() {
  ...
  addPass(createX86SXFIDebugPrePass());
  addPass(createX86SXFIPass());
}
...
void X86PassConfig::addPreEmitPass2() {
  ...
  addPass(createX86SXFIDebugPostPass());
}
