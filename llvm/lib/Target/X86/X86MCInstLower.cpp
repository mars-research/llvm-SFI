...
void X86AsmPrinter::CheckBundleInlineAsm(const MachineInstr *MI){
  ...
  while (next = next->getNextNode()){
    ...
    if(next->isInlineAsm()){
      if (std::string(next->getOperand(0).getSymbolName()).find("%NaclEndBundle")){
        //nacl TODO get the real lenth of the inline asm
        if(32 - OutStreamer->NaClCounter  < bundle_size + 7){
          //errs()<<"emitting padding for ld/st\n";
          OutStreamer->emitCodeAlignment(32, &getSubtargetInfo());
          OutStreamer->reset_counter();
          OutStreamer->Bundle_Started = false;
          return;
        }
      }
    }else{
